...
- Current maintenance changes are source-reviewed, host-tested, and VitaSDK build-verified; firmware-dependent behavior remains hardware-unverified. The "promotion `committed = 1`" assumption (sync=1 promoter calls have installed by return) is well-supported by API contract + independent pkgi/pkgj usage + SceShellSvc IPC architecture — see `docs/agent-context/refresh.md` — but still awaits the staged hardware test for final confirmation.
